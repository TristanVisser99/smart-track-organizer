...
function ui_components.build_header(root, current_palette, on_palette_change)
  local header_panel = root:add(rtk.HBox{
    padding = { 8, 12 },
    valign = "center",
  ...
  local palette_dropdown = palette_box:add(rtk.OptionMenu{
    menu = palette_menu,
...
function ui_components.build_control_row(root, current_options, on_search, on_option_change)
  -- 1. Search Panel Card
  local search_entry
  local search_panel = root:add(rtk.HBox{
    padding = { 8, 12 },
    spacing = 8,
    valign = "center",
    box = { expand = "x", fill = true }
  ...
  search_entry = search_panel:add(rtk.Entry{
  ...
  local toolbar_panel = root:add(rtk.HBox{
    padding = { 8, 12 },
    valign = "center",
    box = { expand = "x", fill = true }
  }, { fillw = true, stretch = rtk.Box.STRETCH_FULL })
  ...
  local chk_folders = options_left_box:add(rtk.CheckBox{ label = "Folder Groups", value = current_options.create_folders, fontscale = theme.typography.body_scale }, { valign = "center" })
  local chk_prefix = options_left_box:add(rtk.CheckBox{ label = "Prefix Codes", value = current_options.prefix_names, fontscale = theme.typography.body_scale }, { valign = "center" })

  toolbar_panel:add(rtk.Box.FLEXSPACE)

  local sort_box = toolbar_panel:add(rtk.HBox{ spacing = 6, valign = "center" }, { valign = "center" })
  sort_box:add(rtk.Text{
    text = "Sort:",
    fontscale = theme.typography.body_scale,
    color = theme.colors.text_secondary
  }, { valign = "center" })
  ...
  local sort_dropdown = sort_box:add(rtk.OptionMenu{
    menu = sort_menu,
...
function ui_components.build_table_header(root, on_toggle_all)
  local table_header = root:add(rtk.HBox{
    padding = 8,
    spacing = 10,
    valign = "center",
    box = { expand = "x", fill = true }
...
function ui_components.build_viewport(root)
  ...
  local viewport = root:add(rtk.Viewport{
    child = table_vbox,
    box = { expand = "both", fill = true },
...
function ui_components.build_status_bar(root)
  local status_bar = root:add(rtk.Text{
    text = "",
    color = theme.colors.text_secondary,
...
function ui_components.build_button_bar(root, on_refresh, on_apply)
  local button_bar = root:add(rtk.HBox{
    spacing = 12,
    halign = "right",
    valign = "center",
    box = { expand = "x", fill = true }
  ...
  local btn_refresh = button_bar:add(rtk.Button{
  ...
  local btn_apply = button_bar:add(rtk.Button{
...
function ui_components.render_preview_rows(table_vbox, display_items, on_row_toggle)
  ...
  for idx, entry in ipairs(display_items) do
    ...
    if item then
      ...
      local row = table_vbox:add(rtk.HBox{
